...
def pretty_print_charger_group(group: ChargerGroup, chargers: Chargers, indent=0, is_last=True, prefix=""):
    ...
    for i, connection in enumerate(group.connections):
        ...
        if isinstance(connection, ChargerGroup):
            ...
            print(f"{new_prefix}{charger_connector}ChargerState(charge_rate_current={chargers.charger_rate_current[charger_idx]}, car_connected={chargers.car_connected[charger_idx]})")
...
#         elif isinstance(connection, ChargerState):
#             charger_connector = "└── " if is_last_connection else "├── "
#             print(f"{new_prefix}{charger_connector}ChargerState(charge_rate_current={connection.charger_rate_current}, car_connected={connection.car_connected})")
...
#             print(f"{new_prefix}{charger_connector}ChargerState(charge_rate_max={charger.charge_rate_max}, charge_rate_current={charger.charge_rate_current}, car={charger.car})")